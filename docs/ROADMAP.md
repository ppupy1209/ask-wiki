...
- **C4 ask-wiki MCP 서버 (신설 2026-07-11, 강의 접목 P3)**: 검색·질의를 **MCP tool(`search_wiki`·`ask_wiki`)로 노출** → Claude Code/Desktop이 위키를 직접 질의하는 데이터 소스가 됨. 서사 = "정합성(B1)·품질 검증(B2)된 검색을 표준 프로토콜로 AI 도구 생태계에 개방". 구현은 `spring-ai-starter-mcp-server-webmvc` — **Boot 3.3.0+ 지원·1.0.x 라인 포함 확인 완료(버전 상향 불필요, C2-1식 함정 없음 — 착수 시 1.0.9 BOM 포함 재확인만)**. 공고 "MCP" 키워드에 훅(W-1)에 이어 실제 제품으로 대응, W-3/W-4 글감과 교차. **순서 = C1 뒤·C3 앞**(1~2세션 경량 페이즈, 2026-07-11 연우님 확정). 강의 섹션 10 레퍼런스.
...
- [ ] Step 2: `VectorIndex` 포트 추출(순수 리팩터링·동작 불변, 기존 스위트 그린 유지) → 새 저장소 compose 추가 → 어댑터 구현 → B1 패턴 이식(relay→포트, 멱등 upsert·문서 삭제) → Ghost/Relay 테스트를 어댑터 구현으로도 그린 **(진행: 2-1 포트 추출 ✅ · 2-2 ES 인프라 ✅ 2026-07-12, Codex 작성·Claude 검증, `635431b`·`a7694e1` — 네이티브 클라이언트 결정은 design-notes §5)**
...
- hit rate@4: 이행 전 93.3% → 이행 후 **동등(±노이즈)** — "회귀 없음" 증명이 헤드라인
