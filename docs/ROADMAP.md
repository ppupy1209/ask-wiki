...
- [ ] **B5-1 설계 결정**: ① ES 매핑에 본문 text 필드 추가 + 색인 경로(`add`/`rebuild`)에 본문 실어 보내기 + 재색인 ② 포트 확장 방식 — `search(text, vector, topK)`로 넓힐지 / 별도 `HybridIndex` 포트를 둘지 / InMemory 구현은 text를 무시할지(BM25 불가)
- [ ] **B5-2 구현**: BM25 질의 + kNN 질의 → **자체 RRF 융합**. `askwiki.search.hybrid` 스위치(기본 off)로 A/B 가능하게 — C3-2의 "기본 off + 결정적 스위트 불변" 선례
...
- hit rate: 기준선 **@1 36.7% · @2 56.7% · @4 93.3% · @8 100%**(B2 실측, 청크 500·topK 4). @4는 천장에 가까우니 **@1·@2가 승부처**.
