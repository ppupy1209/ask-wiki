...
- 🔧 **Phase C2 - 상용 LLM 프로바이더 스위치 (계획 확정, 착수 대기, 2026-07-10)**: B2가 정량적 동기 제공 — 로컬 소형 모델의 환각 바닥(65%/60%)을 상용 모델이 깨는지 B2 하네스로 증명. **확정 결정**: (1) 세 개(Claude/GPT/Gemini) 모두 스위치 가능하게(프로퍼티 `askwiki.llm.provider`), 우선 1개 붙이고 확장 (2) **챗 LLM만 교체**·임베딩 nomic 유지(→ 환각↓ 원인을 LLM 판단으로 귀속하는 실험 통제) (3) B3(SSE) 미포함. 서브스텝 C2-1~C2-5(+배포)는 ROADMAP "Phase C2". 구 B4(장애 격리)·배포 과제 흡수.
    ...
    - **지금 여기 (2026-07-10)**: C2-1~C2-4 완료·커밋(11 커밋, 미푸시). 남은 핵심 = **C2-5**(프로바이더별 환각률 비교 — C2의 하이라이트, "무료 강한 모델이 환각 바닥 65%/60%를 깨는가"를 데이터로 증명). ⚠️ 임베딩은 Gemini 써도 로컬 Ollama(nomic)라 실질의·측정엔 Ollama 가동 필수.
...
- ⬜ **Phase C1·C4·C3 - 채용 공고 대응 딥다이브 (2026-07-07 편성, 2026-07-11 강의 접목 보강)**: C1 벡터 DB 이행(B1 패턴 이식·B2 검증) → **C4 ask-wiki MCP 서버(신설)** → C3 에이전틱 RAG. 근거·매핑은 ROADMAP "채용 공고 대응" 참조. (C2는 위로 승격·착수.)
