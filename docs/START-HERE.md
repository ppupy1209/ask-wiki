...
- 🔧 **Phase C1·C4·C3 - 채용 공고 대응 딥다이브 (2026-07-07 편성, 2026-07-11 강의 접목 보강)**: C1 벡터 DB 이행(B1 패턴 이식·B2 검증) → **C4 ask-wiki MCP 서버(신설)** → C3 에이전틱 RAG. 근거·매핑은 ROADMAP "채용 공고 대응" 참조. (C2는 위로 승격·완료.)
    ...
    - **✅ Step 3-② 20k 스케일 벤치 (2026-07-12)**: 자립형 `VectorIndexScaleBenchTest`(Testcontainers MySQL+ES, 가우시안 랜덤 20k). **지연 역전 이미 20k에서**: 인메모리 avg 22.8ms vs ES kNN 12.8ms(ES 1.8배 빠름 — 브루트포스 O(N) vs HNSW 수백 노드, 네트워크 홉에도 불구). **근사 대가**: recall@4=0.83@nc100, dial(nc 4→0.386 … 200→0.879)로 조절 — 단 가우시안=최악케이스라 실제 임베딩(3-①)은 recall≈완전, 두 측정이 현실을 괄호로 묶음. **부수 발견·수정**: `rebuild()` 무제한 단일 벌크가 20k에서 OOM(가우시안 float 200MB+) → `BULK_BATCH_SIZE=500` 배치(프로덕션 코드 수정, Claude가 벤치 중 발견). design-notes §5.
    - **지금 여기: Step 3 잔여** — ③ 반영 지연(B1 126.7ms + ES refresh 계층) ④ 기동 시간(rebuild 제거 효과 — ES는 영속이라 재시작 재빌드 0, 인메모리는 매 기동 6.3s 실측) ⑤ (강의 접목 P1-②) ETL vs Chunker — B2-5 재실행. 그 뒤 Step 3 마감 → C4(MCP 서버).
