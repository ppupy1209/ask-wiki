...
- 🔧 **Phase C1·C4·C3 - 채용 공고 대응 딥다이브 (2026-07-07 편성, 2026-07-11 강의 접목 보강)**: C1 벡터 DB 이행(B1 패턴 이식·B2 검증) → **C4 ask-wiki MCP 서버(신설)** → C3 에이전틱 RAG. 근거·매핑은 ROADMAP "채용 공고 대응" 참조. (C2는 위로 승격·완료.)
    ...
    - **✅ Step 2-2 완료 (2026-07-12)**: ES 인프라 — compose `elasticsearch:8.17.4`(single-node·보안off·512m힙·es-data 볼륨·`ES_PORT` 오버라이드), `elasticsearch-java`(Boot BOM 관리)+`testcontainers:elasticsearch`, lazy `ElasticsearchClient` 빈, `EsConnectivityTest`(Testcontainers 스모크). **Spring AI VectorStore 스타터 미사용 결정**(add()가 스토어에서 재임베딩하는 설계 — 우리는 임베딩을 MySQL에 보유, design-notes §5). Codex 작성·Claude 검증(diff+전 스위트 그린) `a7694e1`. 위임 중 함정: 서브에이전트 2분 타임아웃이 Codex 프로세스를 함께 죽임(파일 반영은 완료된 뒤였음) → **이후 위임은 --background 모드**로. **지금 여기: Step 2-3(EsVectorIndex 어댑터 — 매핑·멱등 add·kNN 검색·삭제·rebuild + 구현 선택 프로퍼티) Codex 위임 진행.**
