...
- 🔄 **Phase B2 - 답변 품질 평가 하네스 (진행 중, 2026-07-07 시작)**: 결정 = **사내 HR/총무 golden set + JUnit @Tag("eval") 러너**. B2-1 포맷·B2-2 golden set·B2-3 hit rate 러너·B2-5 실험 매트릭스 완료. **hit rate@4=93.3%**, 매트릭스 결과 **청크 500·topK 4 유지 결정**(200자는 파편화로 하락, 400~800 동일 — 짧은 문서 한계). **지금 여기: B2-4 환각률·인용 정확도**(마지막 남은 조각, 채팅 LLM llama3.2 필요) → 이후 B2 완료. ⚠️ golden set 사실관계는 연우님 검수 권장. 세부는 ROADMAP. 진행은 §7 step-by-step.
