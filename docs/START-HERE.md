...
- 🔄 **Phase B2 - 답변 품질 평가 하네스 (진행 중, 2026-07-07 시작)**: 결정 = **사내 HR/총무 golden set + JUnit @Tag("eval") 러너**. B2-1~B2-5 완료. **hit rate@4=93.3%**(청크 500·topK 4 유지), **환각률 65%·오거부율 0%**(llama3.2:3b 기준선). 러너 3종(`HitRate`·`ChunkSizeMatrix`·`Hallucination`, `@Tag("eval")`, `./gradlew evalTest`). **핵심 발견: Phase A 1회 확인은 거짓 안심 — 체계 측정하니 환각 65%.** **지금 여기: B2 마무리 — 프롬프트 강화로 환각률 낮추고 재측정(RagService 프롬프트, 코드 변경)**, 하면 B2 완료. ⚠️ golden set 사실관계는 연우님 검수 권장. 세부는 ROADMAP. 진행은 §7 step-by-step.
