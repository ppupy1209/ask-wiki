...
- 🔧 **Phase C1·C4·C3 - 채용 공고 대응 딥다이브 (2026-07-07 편성, 2026-07-11 강의 접목 보강)**: C1 벡터 DB 이행 ✅ → **C4 ask-wiki MCP 서버(신설, 다음)** → C3 에이전틱 RAG. 근거·매핑은 ROADMAP "채용 공고 대응" 참조. (C2는 위로 승격·완료.)
    ...
    - **✅ C4 완료 (2026-07-13)**: 8파일 신설(`ask/AskOutcome`·`ask/AskService`·`mcp/WikiAnswer`·`mcp/WikiMcpTools`·`mcp/WikiMcpConfig` + 테스트 3)·3파일 수정(build.gradle·application.yml·`ask/AskController`). **전체 스위트 39/39 그린**(Testcontainers MySQL 8.4+ES 8.17.4 풀 컨텍스트). 검증 중 **순환 의존성 발견·수정**: 챗 모델 tool-calling 인프라(`toolCallbackResolver`)가 기동 시 모든 `ToolCallbackProvider`를 즉시 수집 → `wikiTools→WikiMcpTools→AskService→RagService→ChatModel(생성중)` 순환(BeanCurrentlyInCreation) → `WikiMcpTools`의 AskService 주입 `@Lazy`로 절단(런타임 영향 0, **단위테스트는 못 잡고 풀 컨텍스트 부팅이 잡음** = design-notes §6). 구현 Codex·검증/순환수정 Claude(샌드박스가 gradle 막아 Codex가 못 보는 유형). **지금 여기 = C4 구현 완료.** 남음: (선택) 라이브 MCP 데모(스택+Ollama, `/sse` tools/list에 search_wiki·ask_wiki 노출 확인 = W-3/W-4 글감) + 커밋(연우님). 다음 Phase = **C3 에이전틱 RAG**.
